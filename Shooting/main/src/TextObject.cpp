...
int Menu::ShowMenu(SDL_Renderer* des, TTF_Font* font) {
    if (gMenu.LoadImg("img//menu.webp", des) == 0) {
        return 1;
    }
...
int Menu::ShowRestart(SDL_Renderer* des, TTF_Font* font) {
    if (gRestart.LoadImg("img//rs.jpg", des) == 0) {
        return 1;
    }
...
{
     if (tutorial.LoadImg("img//rs.jpg", des) == 0)
     {
        return 1;
     }
    ...
    textMenu[0].SetRect(posArr[0].x, posArr[0].y);
     ...
     guide[1].SetText("Press SPACE to shoot Vegeta");
     ...
     guide[3].SetText("Kill all Vegeta to win!");
